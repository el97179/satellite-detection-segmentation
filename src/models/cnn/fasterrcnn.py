...
class SatelliteFasterRCNNWithKeypoints(SatelliteFasterRCNN):
    """
    Extended Faster R-CNN with keypoint detection for satellite pose estimation.
...
class MultiScaleFasterRCNN(SatelliteFasterRCNN):
    """
    Multi-scale Faster R-CNN for handling satellites at different scales.
...
def create_fasterrcnn_model(config: dict) -> nn.Module:
    ...
    params = {
        ...
        'backbone_name': fasterrcnn_config.get('backbone_name', 'resnet50'),
        'pretrained_backbone': fasterrcnn_config.get('pretrained_backbone', True),
        'trainable_backbone_layers': fasterrcnn_config.get('trainable_backbone_layers', 3),
        'min_size': fasterrcnn_config.get('min_size', 800),
        'max_size': fasterrcnn_config.get('max_size', 1333),
        'rpn_anchor_sizes': tuple(fasterrcnn_config.get('rpn_anchor_sizes', [32, 64, 128, 256, 512])),
        'rpn_aspect_ratios': tuple(fasterrcnn_config.get('rpn_aspect_ratios', [0.5, 1.0, 2.0])),
        'box_score_thresh': fasterrcnn_config.get('box_score_thresh', 0.05),
        'box_nms_thresh': fasterrcnn_config.get('box_nms_thresh', 0.5),
        'box_detections_per_img': fasterrcnn_config.get('box_detections_per_img', 100)
    }
    
    # Check for special variants
    use_keypoints = fasterrcnn_config.get('use_keypoints', False)
    use_multiscale = fasterrcnn_config.get('use_multiscale', False)
    
    if use_keypoints:
        num_keypoints = fasterrcnn_config.get('num_keypoints', 8)
        model_variant = SatelliteFasterRCNNWithKeypoints(num_keypoints=num_keypoints, **params)
    elif use_multiscale:
        scales = fasterrcnn_config.get('scales', [600, 800, 1000, 1200])
